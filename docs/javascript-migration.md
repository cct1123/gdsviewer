...
2. Browser integration (this slice): uploaded `.gds` files are now read as an `ArrayBuffer` and parsed entirely in the browser by `gds_parser.js`; uploads no longer hit `/api/load-gds`, and all layers/groups/templates are present locally instead of fetched per layer from `/api/layer-data`. The Python side still parses preloaded CLI layouts and serves `/api/initial-data` plus `/api/layer-data` for those documents. Browser behavior still needs a manual smoke test (load, visibility, navigation, measurements, grid, scale bar, resize, errors); the automated suite covers asset wiring and endpoint presence only.
3. Static-server reduction: remove parsing/upload/document-store APIs and gdstk/NumPy runtime dependencies; serve only HTML, CSS/embedded styling, parser JavaScript, and renderer JavaScript. Decide and document whether CLI preloading/cell selection remains as browser startup configuration or is removed.
