...
Open a `.gds` chip-layout file, explore its layers, and measure distances in your
browser. Your files stay on your computer.

**No Python, uv, installation, or build step is needed to use the viewer.** The
drawing library is included, so the viewer can work offline. Node.js is only needed
for the optional local server and developer checks below.

![GDS Viewer showing the included sample layout, cell tree, layer controls, and scale bar](docs/images/02-layout-overview.jpg)

*The included example has nine polygons across three layers. The sidebar controls
what you see; the large area on the right displays the layout.*
...
**Testing note:** opening the HTML directly is an intended mode, but it was not
verified because the browser automation blocked local file URLs. The optional
server method below was tested in the Codex in-app browser on Windows.
macOS/Linux launchers and other browser engines remain unverified.
...
and README pictures to remain available with your copy.
...
The tests check saved independent reference models, malformed inputs, and static
asset delivery. See [fixture provenance](tests/fixtures/README.md).
...
MIT. See [LICENSE](LICENSE) and the notices in `vendor/`.
