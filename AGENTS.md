...
- The upload endpoint currently trusts `CONTENT_LENGTH`, reads the entire body into memory, writes a temporary file, parses the full layout, and builds an in-memory view model. It has no upload-size, polygon-count, hierarchy-expansion, repetition-count, time, or memory limit. Small compressed-looking inputs can expand into very large geometry. Do not expose the service to untrusted networks without adding and testing limits.
